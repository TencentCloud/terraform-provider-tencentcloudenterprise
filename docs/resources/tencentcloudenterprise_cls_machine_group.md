...
sidebar_current: "docs-tencentcloudenterprise-resource-cls_machine_group"
description: |-
...
* `cluster_id` - (Optional, String) Cluster ID. When `flag` is `label_tke`, this means the TKE cluster ID. ClusterId and ClusterRegion must both be set or both be empty.
* `cluster_region` - (Optional, String) Cluster region. When `flag` is `label_tke`, this means the TKE cluster region. ClusterId and ClusterRegion must both be set or both be empty.
...
tencentcloudenterprise_cls_machine_group can be imported using the id, e.g.
