...
page_title: "TencentCloudEnterprise: tencentcloudenterprise_cls_ckafka_consumer"
sidebar_current: "docs-tencentcloudenterprise-resource-cls_ckafka_consumer"
description: |-
  Provides a resource to create a cls ckafka_consumer
---

# tencentcloudenterprise_cls_ckafka_consumer
...
* `advanced_config` - (Optional, List) Advanced configuration.
* `ckafka` - (Optional, List) CKafka description.
...
* `partition_fields` - (Optional, List) Fields used for partition hash.
* `partition_hash_status` - (Optional, Bool) Partition hash status for CKafka.
...
tencentcloudenterprise_cls_ckafka_consumer can be imported using the id, e.g.
