...
page_title: "TencentCloudEnterprise: tencentcloudenterprise_clb_listener_rule"
sidebar_current: "docs-tencentcloudenterprise-resource-clb_listener_rule"
description: |-
...
* `health_check_interval_time` - (Optional, Int) Interval time of health check. Valid value ranges: (2~300) sec. and the default is `5` sec. NOTES: TCP/UDP listener allows direct configuration, HTTP/HTTPS listener needs to be configured in `tencentcloudenterprise_clb_listener_rule`.
...
* `scheduler` - (Optional, String) Scheduling method of the CLB listener rules. Valid values: `WRR`, `IP_HASH`, `LEAST_CONN`. The default is `WRR`.  NOTES: TCP/UDP listener allows direct configuration, HTTP/HTTPS listener needs to be configured in `tencentcloudenterprise_clb_listener_rule`.
* `session_expire_time` - (Optional, Int) Time of session persistence within the CLB listener. NOTES: Available when scheduler is specified as `WRR`.  NOTES: TCP/UDP listener allows direct configuration, HTTP/HTTPS listener needs to be configured in `tencentcloudenterprise_clb_listener_rule`.
...
tencentcloudenterprise_clb_listener_rule can be imported using the id, e.g.
