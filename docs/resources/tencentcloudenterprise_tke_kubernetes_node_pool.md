...
sidebar_current: "docs-tencentcloudenterprise-resources-tke_kubernetes_node_pool"
description: |-
...
~> **NOTE:**  We recommend the usage of one cluster with essential worker config + node pool to manage cluster and nodes. Its a more flexible way than manage worker config with tencentcloudenterprise_kubernetes_cluster, tencentcloudenterprise_kubernetes_scale_worker or exist node management of `tencentcloudenterprise_kubernetes_attachment`. Cause some unchangeable parameters of `worker_config` may cause the whole cluster resource `force new`.
