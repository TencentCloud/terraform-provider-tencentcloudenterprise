...
page_title: "TencentCloudEnterprise: tencentcloudenterprise_clb_listeners"
sidebar_current: "docs-tencentcloudenterprise-datasource-clb_listeners"
description: |-
...
* `listener_list` - A list of listeners of cloud load balancers. Each element contains the following attributes:
  ...
  * `health_check_interval_time` - Interval time of health check. The value range is 2-300 sec, and the default is `5` sec. NOTES: TCP/UDP listener allows direct configuration, HTTP/HTTPS listener needs to be configured in tencentcloudenterprise_clb_listener_rule.
