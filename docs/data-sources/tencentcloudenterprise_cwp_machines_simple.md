...
page_title: "TencentCloudEnterprise: tencentcloudenterprise_cwp_machines_simple"
sidebar_current: "docs-tencentcloudenterprise-datasource-cwp_machines_simple"
description: |-
  Use this data source to query detailed information of CWP (Cloud Workload Protection) machines in a simplified format.
---

# tencentcloudenterprise_cwp_machines_simple
...
* `machine_region` - (Required, String) The area where the machine belongs, all-regions: All server region types.
...
* `machines` - Machine list.
  ...
  * `tag` - Tag.
    ...
    * `tag_id` - Tag ID.
  * `tencentcloudenterprise_tags` - Cloud tags detail.Note: This field may return null, indicating that a valid value cannot be obtained.
    * `tag_key` - Tag key.
    * `tag_value` - Tag value.
