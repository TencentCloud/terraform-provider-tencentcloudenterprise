...
page_title: "TencentCloudEnterprise: tencentcloudenterprise_clb_listener_rules"
sidebar_current: "docs-tencentcloudenterprise-datasource-clb_listener_rules"
description: |-
...
* `rule_list` - A list of forward rules of listeners. Each element contains the following attributes:
  ...
  * `health_check_interval_time` - Interval time of health check. The value range is 2-300 sec, and the default is `5` sec. NOTES: TCP/UDP/TCP_SSL listener allows direct configuration, HTTP/HTTPS listener needs to be configured in tencentcloudenterprise_clb_listener_rule.
