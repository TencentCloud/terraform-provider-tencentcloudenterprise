...
resource tencentcloudenterprise_gaap_layer4_listener "foo" {
  protocol        = "TCP"
  ...
  realserver_bind_set {
    ...
    ip     = tencentcloudenterprise_gaap_realserver.foo.ip
    port   = 80
    weight = 1
  }
  ...
  realserver_bind_set {
    ...
    ip     = tencentcloudenterprise_gaap_realserver.bar.ip
    port   = 80
    weight = 2
  }
}

data tencentcloudenterprise_gaap_layer4_listeners "foo" {
  protocol    = "TCP"
  proxy_id    = tencentcloudenterprise_gaap_proxy.foo.id
  listener_id = tencentcloudenterprise_gaap_layer4_listener.foo.id
}