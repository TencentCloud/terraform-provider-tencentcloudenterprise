...
# Query CVM images
data "cloud_cvm_images" "images" {
  image_type = ["PUBLIC_IMAGE"]
  os_name    = "ubuntu"
}

# Query single CVM image
data "cloud_cvm_image" "image" {
  image_id = "img-xxxxx"
}

# Query CVM instance types
data "cloud_cvm_instance_types" "types" {
...
# Query CVM instances
data "cloud_cvm_instances" "instances" {
  instance_id = "ins-xxxxx"
}

# Query CVM instance set
data "cloud_cvm_instances_set" "instance_set" {
  instance_ids = ["ins-xxxxx", "ins-yyyyy"]
}

# Query CVM key pairs
data "cloud_cvm_key_pairs" "keys" {
  key_id = "skey-xxxxx"
}

# Query CVM placement groups
data "cloud_cvm_placement_groups" "groups" {
  placement_group_id = "ps-xxxxx"
}

# Query CVM instances modification
data "cloud_cvm_instances_modification" "modification" {
  instance_ids = ["ins-xxxxx"]
}

# Query CVM instance VNC URL
data "cloud_cvm_instance_vnc_url" "vnc" {
  instance_id = "ins-xxxxx"
}

# Query disaster recover group quota
data "cloud_cvm_disaster_recover_group_quota" "quota" {}

# Query image quota
data "cloud_cvm_image_quota" "quota" {}

# Query image share permission
data "cloud_cvm_image_share_permission" "share" {
  image_id = "img-xxxxx"
}
...
# CVM Placement Group
resource "cloud_cvm_placement_group" "group" {
  name = "example-pg"
  type = "HOST"
}
...
resource "cloud_cvm_instance_set" "instance_set" {
  instance_count             = 3
...
# CVM Launch Template
resource "cloud_cvm_launch_template" "template" {
...
# CVM Image
resource "cloud_cvm_image" "image" {
...
resource "cloud_cvm_security_group_attachment" "sg_attachment" {
  security_group_ids = ["sg-xxxxx"]
  instance_id        = cloud_cvm_instance.instance.id
}

# CVM Reboot Instance
resource "cloud_cvm_reboot_instance" "reboot" {
  instance_id = cloud_cvm_instance.instance.id
  stop_type   = "SOFT"
}

# CVM Renew Instance
resource "cloud_cvm_renew_instance" "renew" {
  instance_id    = cloud_cvm_instance.instance.id
  renew_portable_data_disk = true
}

# CVM Sync Image
resource "cloud_cvm_sync_image" "sync" {
  image_id            = cloud_cvm_image.image.id
  destination_regions = ["ap-shanghai", "ap-beijing"]
}

# CVM Image Share Permission
resource "cloud_cvm_image_share_permission" "share" {
  image_id    = cloud_cvm_image.image.id
