resource "tencentcloudenterprise_teo_zone" "example" {
...
resource "tencentcloudenterprise_teo_zone_setting" "example" {
  zone_id = tencentcloudenterprise_teo_zone.example.id
...
resource "tencentcloudenterprise_teo_dns_record" "rule_record" {
  zone_id = tencentcloudenterprise_teo_zone.example.id
  type    = "A"
...
resource "tencentcloudenterprise_teo_rule_engine" "rule_example" {
  zone_id   = tencentcloudenterprise_teo_zone.example.id
  ...
  rules {
    ...
    or {
      and {
        target   = "host"
        operator = "equal"
        values   = [tencentcloudenterprise_teo_dns_record.rule_record.name]
      }
      and {
        target   = "extension"
        operator = "equal"
