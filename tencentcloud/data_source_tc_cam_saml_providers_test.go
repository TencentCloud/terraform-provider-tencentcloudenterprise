package tencentcloud

import (
	"testing"

	"github.com/hashicorp/terraform-plugin-sdk/v2/helper/resource"
)

const testAccSAMLProviderDataSourceName = "cam-provider-test1"

func TestAccTencentCloudCamSAMLProvidersDataSource_basic(t *testing.T) {
	t.Parallel()
	resource.Test(t, resource.TestCase{
		PreCheck:     func() { testAccPreCheck(t) },
		Providers:    testAccProviders,
		CheckDestroy: testAccCheckCamSAMLProviderDestroy,
		Steps: []resource.TestStep{
			{
				Config: testAccCamSAMLProvidersDataSource_basic,
				Check: resource.ComposeAggregateTestCheckFunc(
					testAccCheckCamSAMLProviderExists("tencentcloudenterprise_cam_saml_provider.provider"),
					resource.TestCheckResourceAttr("data.tencentcloudenterprise_cam_saml_providers.saml_providers", "provider_list.#", "1"),
					resource.TestCheckResourceAttr("data.tencentcloudenterprise_cam_saml_providers.saml_providers", "provider_list.0.name", testAccSAMLProviderDataSourceName),
					resource.TestCheckResourceAttr("data.tencentcloudenterprise_cam_saml_providers.saml_providers", "provider_list.0.description", "test"),
					resource.TestCheckResourceAttrSet("data.tencentcloudenterprise_cam_saml_providers.saml_providers", "provider_list.0.create_time"),
					resource.TestCheckResourceAttrSet("data.tencentcloudenterprise_cam_saml_providers.saml_providers", "provider_list.0.modify_time"),
				),
			},
		},
	})
}

const testAccCamSAMLProvidersDataSource_basic = `
resource "tencentcloudenterprise_cam_saml_provider" "provider" {
  name        = "` + testAccSAMLProviderDataSourceName + `"
  meta_data   = ""
  description = "test"
}

data "tencentcloudenterprise_cam_saml_providers" "saml_providers" {
  name = tencentcloudenterprise_cam_saml_provider.provider.id
}
`
