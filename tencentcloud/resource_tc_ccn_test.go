...
func init() {
	resource.AddTestSweepers("tencentcloudenterprise_ccn", &resource.Sweeper{
		Name: "tencentcloudenterprise_ccn",
...
func TestAccTencentCloudCcnV3Basic(t *testing.T) {
	t.Parallel()
	keyName := "tencentcloudenterprise_ccn.main"
...
func testAccCheckCcnDestroy(s *terraform.State) error {
	...
	for _, rs := range s.RootModule().Resources {
		if rs.Type != "tencentcloudenterprise_ccn" {
			continue
		}
