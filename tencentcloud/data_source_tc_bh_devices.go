...
func init() {
	registerDataDescriptionProvider("tencentcloudenterprise_bh_devices", CNDescription{
		TerraformTypeCN: "BH 设备列表",
		DescriptionCN:   "提供 BH 设备列表数据源，用于查询堡垒机设备信息。",
		AttributesCN: map[string]string{
			...
			"ap_code_set":            "地域编码集合",
			"kind":                   "操作系统类型",
			"authorized_user_id_set": "有访问权限的用户ID集合",
			"resource_id_set":        "资产绑定的堡垒机服务ID集合",
			"kind_set":               "操作系统类型集合",
			"managed_account":        "是否包含托管账号",
			"department_id":          "部门ID",
			"tag_filters":            "标签过滤条件",
			"filters":                "过滤数组",
			"device_set":             "资产信息列表",
							...
							Elem: &schema.Resource{
								Schema: map[string]*schema.Schema{
									...
									"module_set": {
										Type:        schema.TypeSet,
										Computed:    true,
										Description: "Advanced feature list enabled for the service.",
										Elem: &schema.Schema{
											Type: schema.TypeString,
										},
									},
									...
									"share_clb": {
										Type:        schema.TypeBool,
										Computed:    true,
										Description: "Whether to share CLB.",
									},
									"open_clb_id": {
										Type:        schema.TypeString,
										Computed:    true,
									...
									"web_access": {
										Type:        schema.TypeInt,
										Computed:    true,
										Description: "1 default value, web access enabled, 0 web access disabled.",
									},
									"client_access": {
										Type:        schema.TypeInt,
										Computed:    true,
										Description: "1 default value, client access enabled, 0 client access disabled.",
									},
									"external_access": {
										Type:        schema.TypeInt,
										Computed:    true,
										Description: "1 default value, external access enabled, 0 external access disabled.",
									},
									"ioa_resource": {
										Type:        schema.TypeInt,
										Computed:    true,
										Description: "0 default value, 0-free version IOA, 1-paid version IOA.",
									},
									"package_ioa_user_count": {
										Type:        schema.TypeInt,
										Computed:    true,
										Description: "Number of zero trust bastion host user extension packages.",
									},
									"package_ioa_bandwidth": {
										Type:        schema.TypeInt,
										Computed:    true,
										Description: "Number of zero trust bastion host bandwidth extension packages.",
									},
									"ioa_resource_id": {
										Type:        schema.TypeString,
										Computed:    true,
...
func dataSourceTencentCloudBhDevicesRead(d *schema.ResourceData, meta interface{}) error {
	defer logElapsed("data_source.tencentcloudenterprise_bh_devices.read")()
	...
	if v, ok := d.GetOk("tag_filters"); ok {
		...
		tmpSet := make([]*bhsaas.TagFilter, 0, len(tagFiltersSet))
		for _, item := range tagFiltersSet {
			tagFiltersMap := item.(map[string]interface{})
			tagFilter := bhsaas.TagFilter{}
	...
	if v, ok := d.GetOk("filters"); ok {
		...
		tmpSet := make([]*bhsaas.Filter, 0, len(filtersSet))
		for _, item := range filtersSet {
			filtersMap := item.(map[string]interface{})
			filter := bhsaas.Filter{}
	...
	if ok && output.(string) != "" {
		if e := writeToFile(output.(string), deviceSetList); e != nil {
			return e
		}
	}
