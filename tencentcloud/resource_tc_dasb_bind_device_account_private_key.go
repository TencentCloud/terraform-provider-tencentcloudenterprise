...
func init() {
	registerResourceDescriptionProvider("tencentcloudenterprise_dasb_bind_device_account_private_key", CNDescription{
		TerraformTypeCN: "DASB 绑定设备账号私钥",
		DescriptionCN:   "提供 DASB 绑定设备账号私钥资源，用于绑定设备账号私钥。",
...
func resourceTencentCloudDasbBindDeviceAccountPrivateKeyCreate(d *schema.ResourceData, meta interface{}) error {
	defer logElapsed("resource.tencentcloudenterprise_dasb_bind_device_account_private_key.create")()
	defer inconsistentCheck(d, meta)()

	var (
		logId           = getLogId(contextNil)
		request         = bhsaas.NewBindDeviceAccountPrivateKeyRequest()
	...
	err := resource.Retry(writeRetryTimeout, func() *resource.RetryError {
		result, e := meta.(*TencentCloudClient).apiV3Conn.UseBhsaasClient().BindDeviceAccountPrivateKey(request)
		if e != nil {
			return retryError(e)
		} else {
...
func resourceTencentCloudDasbBindDeviceAccountPrivateKeyRead(d *schema.ResourceData, meta interface{}) error {
	defer logElapsed("resource.tencentcloudenterprise_dasb_bind_device_account_private_key.read")()
	defer inconsistentCheck(d, meta)()
...
func resourceTencentCloudDasbBindDeviceAccountPrivateKeyDelete(d *schema.ResourceData, meta interface{}) error {
	defer logElapsed("resource.tencentcloudenterprise_dasb_bind_device_account_private_key.delete")()
	defer inconsistentCheck(d, meta)()
