...
func init() {
	registerResourceDescriptionProvider("tencentcloudenterprise_cam_role", CNDescription{
		TerraformTypeCN: "CAM角色",
		DescriptionCN:   "提供 CAM 角色资源，用于创建和管理 CAM 角色。",
...
func resourceTencentCloudCamRoleCreate(d *schema.ResourceData, meta interface{}) error {
	defer logElapsed("resource.tencentcloudenterprise_cam_role.create")()
	...
	// Validate document shape to avoid backend format errors (must use arrays, no root)
...
func resourceTencentCloudCamRoleRead(d *schema.ResourceData, meta interface{}) error {
	defer logElapsed("resource.tencentcloudenterprise_cam_role.read")()
...
func resourceTencentCloudCamRoleUpdate(d *schema.ResourceData, meta interface{}) error {
	defer logElapsed("resource.tencentcloudenterprise_cam_role.update")()
	...
	descChanged := d.HasChange("description")
	consoleChanged := d.HasChange("console_login")
	if descChanged || consoleChanged {
		description := ""
		if descChanged {
			if v, ok := d.GetOk("description"); ok {
		...
		loginInt := uint64(0)
		if consoleChanged {
			if v, ok := d.GetOkExists("console_login"); ok && v.(bool) {
				loginInt = 1
			}
		}
		...
		if consoleChanged {
			mDescRequest.ConsoleLogin = &loginInt
		}
...
func resourceTencentCloudCamRoleDelete(d *schema.ResourceData, meta interface{}) error {
	defer logElapsed("resource.tencentcloudenterprise_cam_role.delete")()
	...
	err := resource.Retry(writeRetryTimeout, func() *resource.RetryError {
		...
		if e != nil {
			msg := e.Error()
			// tolerate already-missing roles
			if strings.Contains(msg, "RoleNotExist") || strings.Contains(msg, "not exist") || strings.Contains(msg, "NoSuchEntity") {
				return nil
			}
