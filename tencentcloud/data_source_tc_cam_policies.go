...
# query by policy_id
	...
	data "tencentcloudenterprise_cam_policies" "foo" {
	  policy_id = tencentcloudenterprise_cam_policy.foo.id
	}

# query by policy_id and name

	data "tencentcloudenterprise_cam_policies" "bar" {
	  policy_id = tencentcloudenterprise_cam_policy.foo.id
	  name      = "tf-auto-test"
	}
...
func init() {
	registerDataDescriptionProvider("tencentcloudenterprise_cam_policies", CNDescription{
		TerraformTypeCN: "CAM策略列表",
		DescriptionCN:   "用于查询 CAM 策略信息的列表。",
		AttributesCN: map[string]string{
			"name":         "策略名称",
			"policy_id":    "策略 ID",
			"description":  "策略描述",
			"type":         "策略类型 (1: 自定义策略; 2: 预设策略)",
			"create_mode":  "创建模式 (1: 按产品或项目权限创建; 2: 按策略语法创建)",
			"policy_list":  "策略列表",
			"attachments":  "关联实体数量",
			"service_type": "关联产品名称",
			"create_time":  "创建时间",
...
func dataSourceTencentCloudCamPoliciesRead(d *schema.ResourceData, meta interface{}) error {
	defer logElapsed("data_source.tencentcloudenterprise_cam_policies.read")()
