package tencentcloud

import (
	"fmt"
	"log"
	"strings"
	"testing"

	sdkErrors "terraform-provider-tencentcloudenterprise/sdk/common/errors"
	open "terraform-provider-tencentcloudenterprise/sdk/open/v20201202"
	"terraform-provider-tencentcloudenterprise/tencentcloud/internal/helper"

	"github.com/hashicorp/terraform-plugin-sdk/v2/helper/resource"
	"github.com/hashicorp/terraform-plugin-sdk/v2/terraform"
)

const testAccCamSAMLName = "cam-saml-provider-test"

func init() {
	// go test -v ./tencentcloud -sweep=ap-guangzhou -sweep-run=tencentcloudenterprise_cam_saml_provider
	resource.AddTestSweepers("tencentcloudenterprise_cam_saml_provider", &resource.Sweeper{
		Name: "tencentcloudenterprise_cam_saml_provider",
		F: func(r string) error {

			if err := testAccDeleteSAMLProvider(r, testAccCamSAMLName); err != nil {
				log.Printf("Delete SAML Provider %s fail, reason: %s", testAccCamSAMLName, err.Error())
			}
			return nil
		},
	})
}

func testAccDeleteSAMLProvider(region string, name string) error {
	cli, _ := sharedClientForRegion(region)
	client := cli.(*TencentCloudClient).apiV3Conn

	cpf := client.NewClientProfileTce(300)
	openClient, _ := open.NewClient(client.CredentialTce, client.Region, cpf)

	request := open.NewDeleteSAMLProviderRequest()
	request.Name = helper.String(name)

	_, err := openClient.DeleteSAMLProvider(request)
	return err
}

func TestAccTencentCloudCamSAMLProvider_basic(t *testing.T) {
	t.Parallel()
	resource.Test(t, resource.TestCase{
		PreCheck:     func() { testAccPreCheck(t) },
		Providers:    testAccProviders,
		CheckDestroy: testAccCheckCamSAMLProviderDestroy,
		Steps: []resource.TestStep{
			{
				Config: testAccCamSAMLProvider_basic,
				Check: resource.ComposeTestCheckFunc(
					testAccCheckCamSAMLProviderExists("tencentcloudenterprise_cam_saml_provider.saml_provider_basic"),
					resource.TestCheckResourceAttr("tencentcloudenterprise_cam_saml_provider.saml_provider_basic", "name", "cam-saml-provider-test"),
					resource.TestCheckResourceAttr("tencentcloudenterprise_cam_saml_provider.saml_provider_basic", "description", "test"),
				),
			},
			{
				ResourceName:            "tencentcloudenterprise_cam_saml_provider.saml_provider_basic",
				ImportState:             true,
				ImportStateVerify:       true,
				ImportStateVerifyIgnore: []string{"meta_data"},
			},
		},
	})
}

func testAccCheckCamSAMLProviderDestroy(s *terraform.State) error {
	logId := getLogId(contextNil)

	client := testAccProvider.Meta().(*TencentCloudClient).apiV3Conn
	cpf := client.NewClientProfileTce(300)
	openClient, _ := open.NewClient(client.CredentialTce, client.Region, cpf)

	for _, rs := range s.RootModule().Resources {
		if rs.Type != "tencentcloudenterprise_cam_saml_provider" {
			continue
		}

		request := open.NewGetSAMLProviderRequest()
		request.Name = helper.String(rs.Primary.ID)

		instance, err := openClient.GetSAMLProvider(request)
		if err != nil {
			if ee, ok := err.(*sdkErrors.CloudSDKError); ok {
				errCode := ee.GetCode()
				if strings.Contains(errCode, "ResourceNotFound") || strings.Contains(errCode, "NotFound") {
					// Resource not found, this is expected
					continue
				}
			}
			log.Printf("[WARN]%s Error checking SAML provider %s: %s", logId, rs.Primary.ID, err.Error())
			return err
		}

		if instance != nil && instance.Response != nil && instance.Response.Name != nil {
			return fmt.Errorf("[CHECK][CAM SAML provider][Destroy] check: CAM SAML provider still exists: %s", rs.Primary.ID)
		}
	}
	return nil
}

func testAccCheckCamSAMLProviderExists(n string) resource.TestCheckFunc {
	return func(s *terraform.State) error {
		logId := getLogId(contextNil)

		rs, ok := s.RootModule().Resources[n]
		if !ok {
			return fmt.Errorf("[CHECK][CAM SAML provider][Exists] check: CAM SAML provider %s is not found", n)
		}
		if rs.Primary.ID == "" {
			return fmt.Errorf("[CHECK][CAM SAML provider][Exists] check: CAM SAML provider id is not set")
		}

		client := testAccProvider.Meta().(*TencentCloudClient).apiV3Conn
		cpf := client.NewClientProfileTce(300)
		openClient, _ := open.NewClient(client.CredentialTce, client.Region, cpf)

		request := open.NewGetSAMLProviderRequest()
		request.Name = helper.String(rs.Primary.ID)

		instance, err := openClient.GetSAMLProvider(request)
		if err != nil {
			log.Printf("[WARN]%s Error getting SAML provider %s: %s", logId, rs.Primary.ID, err.Error())
			return err
		}
		if instance == nil || instance.Response == nil || instance.Response.Name == nil {
			return fmt.Errorf("[CHECK][CAM SAML provider][Exists] check: CAM SAML provider %s is not exist", rs.Primary.ID)
		}
		return nil
	}
}

const SAMLProviderNameVar = `
variable "saml_provider_name" {
  default = "` + testAccCamSAMLName + `"
}
`

const testAccCamSAMLProvider_basic = SAMLProviderNameVar + `
resource "tencentcloudenterprise_cam_saml_provider" "saml_provider_basic" {
  name        = var.saml_provider_name
  meta_data   = ""
  description = "test"
}
`
