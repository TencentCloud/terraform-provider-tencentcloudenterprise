...
# query by role_id
	...
	data "tencentcloudenterprise_cam_role_policy_attachments" "foo" {
	  role_id = tencentcloudenterprise_cam_role.foo.id
	}

# query by role_id and policy_id

	data "tencentcloudenterprise_cam_role_policy_attachments" "bar" {
	  role_id   = tencentcloudenterprise_cam_role.foo.id
...
func init() {
	registerDataDescriptionProvider("tencentcloudenterprise_cam_role_policy_attachments", CNDescription{
		TerraformTypeCN: "CAM角色策略关联列表",
		DescriptionCN:   "用于查询 CAM 角色关联策略信息的列表。",
		AttributesCN: map[string]string{
			"role_id":                     "角色 ID",
			"policy_id":                   "策略 ID",
			"create_mode":                 "创建模式",
			"policy_type":                 "策略类型",
			"role_policy_attachment_list": "关联策略列表",
			"create_time":                 "创建时间",
			"policy_name":                 "策略名称",
		},
	})
...
func dataSourceTencentCloudCamRolePolicyAttachmentsRead(d *schema.ResourceData, meta interface{}) error {
	defer logElapsed("data_source.tencentcloudenterprise_cam_role_policy_attachments.read")()
