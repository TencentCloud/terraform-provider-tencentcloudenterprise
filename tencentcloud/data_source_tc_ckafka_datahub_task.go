...
//
//```hcl
//data "tencentcloudenterprise_ckafka_datahub_task" "datahub_task" {
//}
//```
//*/
...
//
//func init() {
//	registerDataDescriptionProvider("tencentcloudenterprise_ckafka_datahub_task", CNDescription{
//		TerraformTypeCN: "ckafka_datahub_task",
//		AttributesCN: map[string]string{
//			"search_word":                   "搜索键",
//			"target_type":                   "转储的目标类型",
//			"task_type":                     "任务类型，SOURCE|SINK",
//			"source_type":                   "源类型",
//			"resource":                      "资源",
//			"task_list":                     "Datahub任务信息列表",
//			"task_id":                       "任务ID",
//			"task_name":                     "任务名称",
//			"status":                        "状态，-1创建失败，0创建，1运行，2删除，3删除，4删除失败，5暂停，6暂停，7暂停，8恢复，9恢复失败",
//			"source_resource":               "数据资源",
//			"type":                          "资源类型",
//			"kafka_param":                   "ckafka配置，当类型为KAFKA时需要",
//			"self_built":                    "集群是否是自己构建的，而不是云产品",
//			"topic":                         "主题名称，如果有多个主题，请使用“，”",
//			"offset_type":                   "偏移类型，从开始：最早，从最新：最晚，从特定时间：时间戳",
//			"start_time":                    "当需要偏移类型时间戳时",
//			"resource_name":                 "实例名称",
//			"zone_id":                       "区域ID",
//			"topic_id":                      "主题Id",
//			"partition_num":                 "主题的分区编号",
//			"enable_toleration":             "启用死信队列",
//			"qps_limit":                     "Qps（每秒查询次数）限制",
//			"table_mappings":                "表到主题的映射，选择多主题时需要",
//			"database":                      "数据库名称",
//			"table":                         "表名称",
//			"use_table_mapping":             "是否使用多表",
//			"use_auto_create_topic":         "使用的主题是否需要自动创建（当前仅支持SOURCE流入任务，如果不使用分发到多个主题，则需要在“主题”字段中填写需要自动创建的主题名称）",
//			"compression_type":              "写入Topic时是否压缩，如果未启用，则填写none，如果启用，则填充open",
//			"msg_multiple":                  "1条源主题消息被放大为msg Multiple并写入目标主题（该参数目前仅适用于流入ckafka的ckafka）",
//			"connector_sync_type":           "ConnectorSyncType",
//			"keep_partition":                "KeepPartition",
//			"event_bus_param":               "EB配置，当类型为EB时需要",
//			"namespace":                     "SCF命名空间",
//			"function_name":                 "SCF函数名称",
//			"qualifier":                     "SCF版本和别名",
//			"mongo_db_param":                "MongoDB配置，当类型为MongoDB时需要",
//			"collection":                    "MongoDB集合",
//			"copy_existing":                 "是否复制库存数据，默认参数为true",
//			"ip":                            "Mongo DB连接ip",
//			"port":                          "MongoDB连接端口",
//			"user_name":                     "MongoDB数据库用户名",
//			"password":                      "MongoDB数据库密码",
//			"listening_event":               "侦听事件类型，如果为空，则表示全选值包括insert、update、replace、delete、invalide、drop、dropdatabase、rename，在多个类型之间使用，用逗号分隔",
//			"read_preference":               "主从优先级，默认主节点",
//			"pipeline":                      "聚合管道",
//			"es_param":                      "Es配置，当类型为Es时需要",
//			"service_vip":                   "实例vip",
//			"uniq_vpc_id":                   "实例vpc id",
//			"drop_invalid_message":          "Es是否丢弃解析失败的消息",
//			"index":                         "Es索引名称",
//			"date_format":                   "Es日期后缀",
//			"content_key":                   "非json格式数据的键",
//			"drop_invalid_json_message":     "Es是否丢弃非json格式的消息",
//			"document_id_field":             "转储到Es中的文档ID值的字段名",
//			"index_type":                    "Es自定义索引名称类型，STRING，JSONPATH，默认值为STRING",
//			"drop_cls":                      "当成员参数Drop Invalid Message To Cls设置为true时，Drop Invite Message参数无效",
//			"drop_invalid_message_to_cls":   "是否交付给cls",
//			"drop_cls_region":               "交付cls的区域",
//			"drop_cls_owneruin":             "cls的交货帐户",
//			"drop_cls_topic_id":             "cls的主题",
//			"drop_cls_log_set":              "cls日志集",
//			"database_primary_key":          "当转储到ES的消息是Database的binlog时，如果需要同步数据库操作，即在向ES添加、删除和修改操作时，填写数据库表的主键",
//			"drop_dlq":                      "死信队列",
//			"retry_interval":                "重试间隔",
//			"max_retry_attempts":            "重试次数",
//			"dlq_type":                      "dlq类型，CKAFKA|TOPIC",
//			"tdw_param":                     "Tdw配置，当类型为Tdw时需要",
//			"bid":                           "Tdw出价",
//			"tid":                           "Tdw tid",
//			"is_domestic":                   "默认为true",
//			"tdw_host":                      "TDW地址，默认为tl-tdbank-tdmanager.tencent-distribute.com",
//			"tdw_port":                      "TDW端口，默认为8099",
//			"dts_param":                     "Dts配置，当类型为Dts时需要",
//			"group_id":                      "Dts消费者组Id",
//			"group_user":                    "Dts账户",
//			"group_password":                "Dts使用者组密码",
//			"tran_sql":                      "False用于同步原始数据，true用于同步解析的json格式数据，默认值为true",
//			"click_house_param":             "单击House config，键入ClickHouse required",
//			"cluster":                       "单击House群集",
//			"schema":                        "单击House架构",
//			"column_name":                   "列名",
//			"json_key":                      "与该列对应的json密钥名称",
//			"allow_null":                    "列项目是否允许为空",
//			"cls_param":                     "Cls配置，当类型为Cls时需要",
//			"decode_json":                   "生成的信息是否为json格式",
//			"log_set":                       "日志集id",
//			"time_field":                    "将消息中某个字段的内容指定为cls日志的时间字段内容的格式需要是二级时间戳",
//			"cos_param":                     "Cos配置，当类型为Cos时需要",
//			"bucket_name":                   "cos存储桶名称",
//			"region":                        "地区代码",
//			"object_key":                    "ObjectKey",
//			"aggregate_batch_size":          "聚合邮件的大小MB",
//			"aggregate_interval":            "时间间隔",
//			"format_output_type":            "消息聚合后的文件格式csv|json",
//			"object_key_prefix":             "转储的对象目录前缀",
//			"directory_time_format":         "根据strptime时间格式化的分区格式",
//			"my_sql_param":                  "MySQL配置，当类型为MySQL时需要",
//			"snapshot_mode":                 "是否复制库存信息（schema_only不复制，初始全额），默认为初始",
//			"ddl_topic":                     "存储My SQL的Ddl信息的Topic，如果为空，则默认情况下不会存储",
//			"data_source_monitor_mode":      "TABLE表示读取的项目是一个表，QUERY表示读取的项是一个查询",
//			"data_source_monitor_resource":  "当DataMonitorMode=TABLE时，传入需要读取的表；当DataMonitorMode=QUERY时，传入需要读取的查询sql语句",
//			"data_source_increment_mode":    "TIMESTAMP表示增量列为时间戳类型，INCREMENT表示增量列是自增量id类型&#39；",
//			"data_source_increment_column":  "要监视的列的名称",
//			"data_source_start_from":        "HEAD表示复制库存+增量数据，TAIL表示仅复制增量数据",
//			"data_target_insert_mode":       "INSERT表示使用INSERT模式插入，UPSERT表示使用UPSERT模式插入",
//			"data_target_primary_key_field": "当DataInsertMode=UPSERT时，传入当前upstart所依赖的主键",
//			"data_target_record_mapping":    "表和消息之间的映射关系",
//			"extra_info":                    "数据库表的额外字段",
//			"column_size":                   "当前列大小",
//			"decimal_digits":                "当前列精度",
//			"auto_increment":                "是否为自动递增列",
//			"default_数据库表默认参数":              "数据库表默认参数",
//			"topic_regex":                   "用于将事件路由到特定主题的正则表达式，默认为（.*）",
//			"topic_replacement":             "TopicRegex，$1，$2",
//			"key_columns":                   "格式库1.table1字段1，字段2；library2.table2字段2，位于表之间；（分号）分隔，字段之间，（逗号）分隔未指定的表默认为该表的主键",
//			"output_format":                 "输出格式，DEFAULT，CANAL_1，CANAL_2",
//			"is_table_prefix":               "当Table输入是前缀时，该项的值为true，否则为false",
//			"include_content_changes":       "如果该值为all，DDL数据和DML数据也将写入所选主题；如果值为dml，则只会将dml数据写入所选主题",
//			"include_query":                 "如果该值为true，并且My SQL中binlog行查询日志事件配置项的值为ON，则流入主题的数据包含原始SQL语句；如果该值为false，则流入主题的数据不包含原始SQL语句",
//			"record_with_schema":            "如果值为true，则消息将携带与消息结构相对应的架构，如果值为false，则不会携带",
//			"signal_database":               "信号表的数据库名称",
//			"is_table_regular":              "无论输入表是否为正则表达式，如果该选项和“是否为表前缀”同时为true，则该选项的判断优先级高于“是否为表格前缀”",
//			"postgre_sql_param":             "PostgreSQL配置，当类型为PostgreSQL或TDSQL C_POSTGRESSQL时需要",
//			"plugin_name":                   "（decoderbufs/pgoutput），默认的decoderbufs",
//			"data_format":                   "上游数据格式（JSON|Debezium），当数据库同步模式与默认字段匹配时需要",
//			"topic_param":                   "主题配置，当类型为“主题”时需要",
//			"maria_db_param":                "MariaDB配置，当类型为MariaDB时需要",
//			"sql_server_param":              "SQLServer配置，当类型为SQLServer时需要",
//			"scf_param":                     "Scf配置，当类型为Scf时需要",
//			"batch_size":                    "每个批次中发送的最大消息数，默认值为1000",
//			"max_retries":                   "SCF调用失败后的重试次数，默认为5次",
//			"target_resource":               "目标资源",
//			"ctsdb_param":                   "Ctsdb配置，当类型为Ctsdb时需要",
//			"ctsdb_metric":                  "Ctsdb度量",
//			"create_time":                   "CreateTime",
//			"error_message":                 "错误消息",
//			"task_progress":                 "创建进度百分比",
//			"task_current_step":             "任务当前步骤",
//			"datahub_id":                    "数据集线器Id",
//			"step_list":                     "步骤列表",
...
//		Description: "Tencent Cloud Ckafka Datahub Task",
...
//	defer logElapsed("data_source.tencentcloudenterprise_ckafka_datahub_task.read")()
