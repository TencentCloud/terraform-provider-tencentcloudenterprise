...
```hcl
data "tencentcloudenterprise_tcr_vpc_attachments" "id" {
  instance_id 			= "tcr-satg5125"
}
```
*/
...
func init() {
	registerDataDescriptionProvider("tencentcloudenterprise_tcr_vpc_attachments", CNDescription{
		TerraformTypeCN: "TCR VPC接入",
		DescriptionCN:   "提供TCR VPC接入数据源，用于查询TCR VPC接入的详细信息。",
		AttributesCN: map[string]string{
			"instance_id":        "要查询的实例ID",
			"vpc_id":             "私有网络ID",
			"subnet_id":          "子网ID",
			"result_output_file": "用于保存结果",
			"vpc_attachment_list": "VPC接入列表",
			"access_ip":          "内网接入IP",
			"status":             "VPC接入状态",
		},
	})
...
func dataSourceTencentCloudTCRVPCAttachments() *schema.Resource {
	return &schema.Resource{
		...
		Description: "Use this data source to query detailed information of TCR VPC attachments",
...
func dataSourceTencentCloudTCRVPCAttachmentsRead(d *schema.ResourceData, meta interface{}) error {
	defer logElapsed("data_source.tencentcloudenterprise_tcr_vpc_attachments.read")()
