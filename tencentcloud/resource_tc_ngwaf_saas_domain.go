...
func resourceTencentCloudNgwafSaasDomain() *schema.Resource {
	return &schema.Resource{
		Create: resourceTencentCloudNgwafSaasDomainCreate,
		Read:   resourceTencentCloudNgwafSaasDomainRead,
		Update: resourceTencentCloudNgwafSaasDomainUpdate,
		Delete: resourceTencentCloudNgwafSaasDomainDelete,
...
func resourceTencentCloudNgwafSaasDomainCreate(d *schema.ResourceData, meta interface{}) error {
	defer logElapsed("resource.tencentcloudenterprise_ngwaf_saas_domain.create")()
	...
	var (
		...
		verifyRequest    = ngwaf.NewDescribeDomainVerifyResultRequest()
		request          = ngwaf.NewAddSpartaProtectionRequest()
		instanceID       string
		domain           string
		domainId         string
		loadBalance      string
	...
	err := resource.Retry(writeRetryTimeout, func() *resource.RetryError {
		result, e := meta.(*TencentCloudClient).apiV3Conn.UseNgwafClient().DescribeDomainVerifyResult(verifyRequest)
		if e != nil {
			return retryError(e)
		} else {
	...
	err = resource.Retry(writeRetryTimeout, func() *resource.RetryError {
		result, e := meta.(*TencentCloudClient).apiV3Conn.UseNgwafClient().AddSpartaProtection(request)
		if e != nil {
			return retryError(e)
		} else {
			log.Printf("[DEBUG]%s api[%s] success, request body [%s], response body [%s]\n", logId, request.GetAction(), request.ToJsonString(), result.ToJsonString())
	...
	if v, ok := d.GetOkExists("api_safe_status"); ok {
		...
		if tmpApiSafeStatus == API_SAFE_STATUS_1 {
			...
			modifyApiAnalyzeStatusRequest := ngwaf.NewModifyApiAnalyzeStatusRequest()
			...
			err = resource.Retry(writeRetryTimeout, func() *resource.RetryError {
				result, e := meta.(*TencentCloudClient).apiV3Conn.UseNgwafClient().ModifyApiAnalyzeStatus(modifyApiAnalyzeStatusRequest)
				if e != nil {
					return retryError(e)
				} else {
	...
	if v, ok := d.GetOkExists("cls_status"); ok {
		...
		if tmpClsStatus == CLS_STATUS_1 {
			...
			modifyDomainsCLSStatusRequest := ngwaf.NewModifyDomainsCLSStatusRequest()
			modifyDomainsCLSStatusRequest.Domains = []*ngwaf.DomainURI{
				{
					Domain:     helper.String(domain),
					Edition:    helper.String("sparta-waf"),
			...
			err = resource.Retry(writeRetryTimeout, func() *resource.RetryError {
				result, e := meta.(*TencentCloudClient).apiV3Conn.UseNgwafClient().ModifyDomainsCLSStatus(modifyDomainsCLSStatusRequest)
				if e != nil {
					return retryError(e)
				} else {
	...
	if v, ok := d.GetOkExists("status"); ok {
		...
		if tmpProtectionStatus == PROTECTION_STATUS_0 {
			...
			modifyProtectionStatusRequest := ngwaf.NewModifyProtectionStatusRequest()
			modifyProtectionStatusRequest.Domain = &domain
			modifyProtectionStatusRequest.Status = &protectionStatus

			err = resource.Retry(writeRetryTimeout, func() *resource.RetryError {
				result, e := meta.(*TencentCloudClient).apiV3Conn.UseNgwafClient().ModifyProtectionStatus(modifyProtectionStatusRequest)
				if e != nil {
					return retryError(e)
				} else {
	...
	return resourceTencentCloudNgwafSaasDomainRead(d, meta)
}

func resourceTencentCloudNgwafSaasDomainRead(d *schema.ResourceData, meta interface{}) error {
	defer logElapsed("resource.tencentcloudenterprise_ngwaf_saas_domain.read")()
	defer inconsistentCheck(d, meta)()
...
func resourceTencentCloudNgwafSaasDomainUpdate(d *schema.ResourceData, meta interface{}) error {
	defer logElapsed("resource.tencentcloudenterprise_ngwaf_saas_domain.update")()
	defer inconsistentCheck(d, meta)()
	...
	var (
		...
		ctx           = context.WithValue(context.TODO(), logIdKey, logId)
		service       = &NgwafService{client: meta.(*TencentCloudClient)}
		request       = ngwaf.NewModifySpartaProtectionRequest()
		botStatus     uint64
		apiSafeStatus uint64
		clsStatus     uint64
		//ipv6Status    int64
	...
	if v, ok := d.GetOk("ports"); ok {
		...
		for _, item := range resPort {
			dMap := item.(map[string]interface{})
			portItem := ngwaf.SpartaProtectionPort{}
			if v, ok := dMap["port"]; ok {
				portItem.Port = helper.String(v.(string))
			}
	...
	err = resource.Retry(writeRetryTimeout, func() *resource.RetryError {
		result, e := meta.(*TencentCloudClient).apiV3Conn.UseNgwafClient().ModifySpartaProtection(request)
		if e != nil {
			return retryError(e)
		} else {
			log.Printf("[DEBUG]%s api[%s] success, request body [%s], response body [%s]\n", logId, request.GetAction(), request.ToJsonString(), result.ToJsonString())
...
func resourceTencentCloudNgwafSaasDomainDelete(d *schema.ResourceData, meta interface{}) error {
	defer logElapsed("resource.tencentcloudenterprise_ngwaf_saas_domain.delete")()
	defer inconsistentCheck(d, meta)()
