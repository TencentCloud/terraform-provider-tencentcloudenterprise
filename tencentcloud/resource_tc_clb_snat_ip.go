...
```hcl
resource "tencentcloudenterprise_clb_instance" "snat_test" {
  network_type = "OPEN"
  clb_name     = "tf-clb-snat-test"
}

resource "tencentcloudenterprise_clb_snat_ip" "foo" {
  clb_id = tencentcloudenterprise_clb_instance.snat_test.id
  ips {
...
```
$ terraform import tencentcloudenterprise_clb_snat_ip.test clb_id
```
...
func resourceTencentCloudClbSnatIpRead(d *schema.ResourceData, meta interface{}) error {
	defer logElapsed("resource.tencentcloudenterprise_clb_snat_ip.read")()
...
func resourceTencentCloudClbSnatIpCreate(d *schema.ResourceData, meta interface{}) error {
	defer logElapsed("resource.tencentcloudenterprise_clb_snat_ip.create")()
...
func resourceTencentCloudClbSnatIpUpdate(d *schema.ResourceData, meta interface{}) error {
	defer logElapsed("resource.tencentcloudenterprise_clb_snat_ip.update")()
	logId := getLogId(contextNil)
	ctx := context.WithValue(context.TODO(), logIdKey, logId)
	clbId := d.Id()
	client := meta.(*TencentCloudClient).apiV3Conn
...
func resourceTencentCloudClbSnatIpDelete(d *schema.ResourceData, meta interface{}) error {
	defer logElapsed("resource.tencentcloudenterprise_clb_snat_ip.delete")()
