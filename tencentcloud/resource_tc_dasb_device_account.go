...
func init() {
	registerResourceDescriptionProvider("tencentcloudenterprise_dasb_device_account", CNDescription{
		TerraformTypeCN: "DASB 设备账号",
		DescriptionCN:   "提供 DASB 设备账号资源，用于创建和管理设备账号。",
...
func resourceTencentCloudDasbDeviceAccountCreate(d *schema.ResourceData, meta interface{}) error {
	defer logElapsed("resource.tencentcloudenterprise_dasb_device_account.create")()
	defer inconsistentCheck(d, meta)()

	var (
		logId           = getLogId(contextNil)
		request         = bhsaas.NewCreateDeviceAccountRequest()
		response        = bhsaas.NewCreateDeviceAccountResponse()
	...
	err := resource.Retry(writeRetryTimeout, func() *resource.RetryError {
		result, e := meta.(*TencentCloudClient).apiV3Conn.UseBhsaasClient().CreateDeviceAccount(request)
		if e != nil {
			return retryError(e)
		} else {
...
func resourceTencentCloudDasbDeviceAccountRead(d *schema.ResourceData, meta interface{}) error {
	defer logElapsed("resource.tencentcloudenterprise_dasb_device_account.read")()
	defer inconsistentCheck(d, meta)()
...
func resourceTencentCloudDasbDeviceAccountDelete(d *schema.ResourceData, meta interface{}) error {
	defer logElapsed("resource.tencentcloudenterprise_dasb_device_account.delete")()
	defer inconsistentCheck(d, meta)()
