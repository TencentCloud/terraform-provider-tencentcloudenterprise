...
func TestAccTencentCloudCcnV3AttachmentBasic(t *testing.T) {
	t.Parallel()
	keyName := "tencentcloudenterprise_ccn_attachment.attachment"
	keyNameVpngw := "tencentcloudenterprise_ccn_attachment.vpngw_ccn_attachment"
...
func testAccCheckCcnAttachmentDestroy(s *terraform.State) error {
	...
	for _, rs := range s.RootModule().Resources {
		if rs.Type != "tencentcloudenterprise_ccn_attachment" {
			continue
		}
...
resource tencentcloudenterprise_ccn_attachment attachment {
...
resource tencentcloudenterprise_vpn_gateway ccn_vpngw {
...
resource tencentcloudenterprise_ccn vpngw_ccn_main {
  name        = "ci-temp-test-vpngw-ccn"
  description = "ci-temp-test-vpngw-ccn-des"
  qos         = "AG"
}

resource tencentcloudenterprise_ccn_attachment vpngw_ccn_attachment {
  ccn_id          = tencentcloudenterprise_ccn.vpngw_ccn_main.id
  instance_type   = "VPNGW"
  instance_id     = tencentcloudenterprise_vpn_gateway.ccn_vpngw.id
  instance_region = var.region
}
`
