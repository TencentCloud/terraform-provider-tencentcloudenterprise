...
func init() {
	registerDataDescriptionProvider("tencentcloudenterprise_cam_group_policy_attachments", CNDescription{
		TerraformTypeCN: "CAM用户组策略关联列表",
		DescriptionCN:   "用于查询 CAM 用户组关联策略信息的列表。",
		AttributesCN: map[string]string{
			"group_id":                     "用户组 ID",
			"policy_id":                    "策略 ID",
			"create_mode":                  "创建模式",
			"policy_type":                  "策略类型",
			"group_policy_attachment_list": "关联策略列表",
			"create_time":                  "创建时间",
			"policy_name":                  "策略名称",
		},
	})
...
func dataSourceTencentCloudCamGroupPolicyAttachmentsRead(d *schema.ResourceData, meta interface{}) error {
	defer logElapsed("data_source.tencentcloudenterprise_cam_group_policy_attachments.read")()
