...
```hcl
	...
	resource "tencentcloudenterprise_cam_role_by_name" "foo" {
	  name          = "cam-role-test"
	  document      = <<EOF
...
```hcl
	...
	resource "tencentcloudenterprise_cam_role_by_name" "boo" {
	  name          = "cam-role-test"
	  document      = <<EOF
...
```
$ terraform import tencentcloudenterprise_cam_role_by_name.foo cam-role-test
```
*/
...
func init() {
	registerResourceDescriptionProvider("tencentcloudenterprise_cam_role_by_name", CNDescription{
		TerraformTypeCN: "CAM角色通过名称",
		DescriptionCN:   "提供 CAM 角色资源，允许通过角色名称来管理 CAM 角色。",
...
func resourceTencentCloudCamRoleByNameCreate(d *schema.ResourceData, meta interface{}) error {
	defer logElapsed("resource.tencentcloudenterprise_cam_role_by_name.create")()
...
func resourceTencentCloudCamRoleByNameRead(d *schema.ResourceData, meta interface{}) error {
	defer logElapsed("resource.tencentcloudenterprise_cam_role_by_name.read")()
...
func resourceTencentCloudCamRoleByNameUpdate(d *schema.ResourceData, meta interface{}) error {
	defer logElapsed("resource.tencentcloudenterprise_cam_role_by_name.update")()
	...
	if d.HasChange("description") {
		...
		// ConsoleLogin is required by the API even when updating description
		if v, ok := d.GetOkExists("console_login"); ok {
			loginBool := v.(bool)
			loginInt := uint64(1)
			if !loginBool {
...
func resourceTencentCloudCamRoleByNameDelete(d *schema.ResourceData, meta interface{}) error {
	defer logElapsed("resource.tencentcloudenterprise_cam_role_by_name.delete")()
