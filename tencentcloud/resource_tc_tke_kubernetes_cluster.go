...
func resourceTencentCloudTkeClusterUpdate(d *schema.ResourceData, meta interface{}) error {
	...
	if d.HasChange("master_config") {
		...
		newNames := make([]string, 0, len(newMap))
		...
		for _, name := range newNames {
			...
			if !exists {
				...
				if field := masterConfigMutationField(oldM, newM); field != "" {
					return fmt.Errorf("modifying field %q of existing master_config block %q is not supported. Please delete the block and add a new one instead", field, name)
				}
			}
		}
