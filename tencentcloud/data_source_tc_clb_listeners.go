...
func init() {
	registerDataDescriptionProvider("tencentcloudenterprise_clb_listeners", CNDescription{
		...
		AttributesCN: map[string]string{
			...
			"health_check_interval_time": "健康检查的间隔时间取值范围为2-300秒，默认为“5”秒注意：TCP/UDP侦听器允许直接配置，HTTP/HTTPS侦听器需要在tencentcloudenterprise_clb_listener_rule中配置",
			"health_check_health_num":    "健康检查的健康阈值，默认为“3”如果连续三次返回健康检查的成功结果，则CVM被识别为健康数值范围为2-10注意：TCP/UDP侦听器允许直接配置，HTTP/HTTPS侦听器需要在tencentcloudenterprise_clb_listener_rule中配置",
			"health_check_unhealth_num":  "健康检查的不健康阈值，默认值为“3”如果连续三次返回健康检查的成功结果，则CVM被识别为不健康数值范围为2-10注意：TCP/UDP侦听器允许直接配置，HTTP/HTTPS侦听器需要在tencentcloudenterprise_clb_listener_rule中配置",
			...
			"session_expire_time":        "CLB侦听器中的会话持续时间注意：TCP/UDP侦听器允许直接配置，HTTP/HTTPS侦听器需要在tencentcloudenterprise_clb_listener_rule中配置",
			"scheduler":                  "CLB侦听器的调度方法，可用值为“WRR”和“LEAST_CONN”默认值为“WRR”注意：“HTTP”和“HTTPS”协议的侦听器还支持“IP HASH”方法注意：TCP/UDP侦听器允许直接配置，HTTP/HTTPS侦听器需要在tencentcloudenterprise_clb_listener_rule中配置",
