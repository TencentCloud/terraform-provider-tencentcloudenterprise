...
```hcl
	...
	resource "tencentcloudenterprise_tcr_instance" "foo" {
	  name              = "example"
	  instance_type		= "basic"
...
```
$ terraform import tencentcloudenterprise_tcr_instance.foo cls-cda1iex1
```
*/
...
func init() {
	registerResourceDescriptionProvider("tencentcloudenterprise_tcr_instance", CNDescription{
		TerraformTypeCN: "TCR实例",
		DescriptionCN:   "提供TCR实例资源，用于创建和管理容器镜像仓库实例",
		AttributesCN: map[string]string{
			"name":                                "实例名称",
			"instance_type":                       "实例类型",
			"tags":                                "标签",
			"delete_bucket":                       "删除存储桶",
			"open_public_operation":               "开启公网操作",
...
func resourceTencentCloudTcrInstance() *schema.Resource {
	return &schema.Resource{
		...
		Description: "Provides a resource to create and manage TCR instance",
		...
		Schema: map[string]*schema.Schema{
			...
			"replications": {
				...
				Optional:    true,
				Description: "Specify List of instance Replications, premium only.",
				Elem: &schema.Resource{
					Schema: map[string]*schema.Schema{
						"id": {
							Type:        schema.TypeString,
...
func resourceTencentCloudTcrInstanceCreate(d *schema.ResourceData, meta interface{}) error {
	defer logElapsed("resource.tencentcloudenterprise_tcr_instance.create")()
...
func resourceTencentCloudTcrInstanceRead(d *schema.ResourceData, meta interface{}) error {
	defer logElapsed("resource.tencentcloudenterprise_tcr_instance.read")()
...
func resourceTencentCloudTcrInstanceUpdate(d *schema.ResourceData, meta interface{}) error {
	defer logElapsed("resource.tencentcloudenterprise_tcr_instance.update")()
	//delete_bucket
	var (
		outErr, inErr error
		instanceId    string
...
func resourceTencentCloudTcrInstanceDelete(d *schema.ResourceData, meta interface{}) error {
	defer logElapsed("resource.tencentcloudenterprise_tcr_instance.delete")()
	...
	for i := range replicas {
		...
		_ = resource.Retry(writeRetryTimeout*5, func() *resource.RetryError {
			...
			if err != nil {
				return retryError(err, "InternalError.ErrorConflict")
			}
			return nil
		})
	}
			...
			err := resource.Retry(writeRetryTimeout*5, func() *resource.RetryError {
				...
				if err != nil {
					...
					if ok {
						...
						if code == "FailedOperation" {
							return resource.NonRetryableError(sdkErr)
						}
						if code == "InternalError.ErrorConflict" {
							return resource.RetryableError(err)
						}
						if code == "InternalError" && strings.Contains(message, "409 InvalidBucketState") {
	...
	if list := removes.List(); removes.Len() > 0 {
		for i := range list {
			...
			err := resource.Retry(writeRetryTimeout*5, func() *resource.RetryError {
				...
				if err != nil {
					return retryError(err, "InternalError.ErrConflict")
				}
				return nil
			})
			if err != nil {
