...
// info  for one direct connect gateway (used by data sources).
...
func (me *VpcService) DescribeDirectConnectGateway(ctx context.Context, dcgId string) (info DcgInstanceInfo, has int, errRet error) {
	...
	infos, err := me.DescribeDirectConnectGatewaysByFilter(ctx, dcgId, "")
	if err != nil {
		errRet = err
		return
	}
...
func (me *VpcService) DescribeDirectConnectGatewaysByFilter(ctx context.Context, dcgId, name string) (
...
func (me *VpcService) DescribeDirectConnectGatewayCcnRoutes(ctx context.Context, dcgId, ccnRouteType string) (infos []DcgRouteInfo, errRet error) {
	logId := getLogId(ctx)
