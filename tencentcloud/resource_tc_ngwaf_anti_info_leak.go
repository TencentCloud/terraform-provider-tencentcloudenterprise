...
func resourceTencentCloudNgwafAntiInfoLeak() *schema.Resource {
	return &schema.Resource{
		Create: resourceTencentCloudNgwafAntiInfoLeakCreate,
		Read:   resourceTencentCloudNgwafAntiInfoLeakRead,
		Update: resourceTencentCloudNgwafAntiInfoLeakUpdate,
		Delete: resourceTencentCloudNgwafAntiInfoLeakDelete,
...
func resourceTencentCloudNgwafAntiInfoLeakCreate(d *schema.ResourceData, meta interface{}) error {
	defer logElapsed("resource.tencentcloudenterprise_ngwaf_anti_info_leak.create")()
	defer inconsistentCheck(d, meta)()

	var (
		logId    = getLogId(contextNil)
		request  = ngwaf.NewAddAntiInfoLeakRulesRequest()
		response = ngwaf.NewAddAntiInfoLeakRulesResponse()
		ruleId   string
		domain   string
	)
	...
	if v, ok := d.GetOk("strategies"); ok {
		for _, item := range v.([]interface{}) {
			...
			strategyForAntiInfoLeak := ngwaf.StrategyForAntiInfoLeak{}
	...
	err := resource.Retry(writeRetryTimeout, func() *resource.RetryError {
		result, e := meta.(*TencentCloudClient).apiV3Conn.UseNgwafClient().AddAntiInfoLeakRules(request)
		if e != nil {
			return retryError(e)
		} else {
	...
	if v, ok := d.GetOkExists("status"); ok {
		...
		if status != ANTI_INFO_LEAK_RULE_STATUS_1 {
			modifyAntiInfoLeakRuleStatusRequest := ngwaf.NewModifyAntiInfoLeakRuleStatusRequest()
			...
			err = resource.Retry(writeRetryTimeout, func() *resource.RetryError {
				result, e := meta.(*TencentCloudClient).apiV3Conn.UseNgwafClient().ModifyAntiInfoLeakRuleStatus(modifyAntiInfoLeakRuleStatusRequest)
				if e != nil {
					return retryError(e)
				} else {
	...
	return resourceTencentCloudNgwafAntiInfoLeakRead(d, meta)
}

func resourceTencentCloudNgwafAntiInfoLeakRead(d *schema.ResourceData, meta interface{}) error {
	defer logElapsed("resource.tencentcloudenterprise_ngwaf_anti_info_leak.read")()
	defer inconsistentCheck(d, meta)()
...
func resourceTencentCloudNgwafAntiInfoLeakUpdate(d *schema.ResourceData, meta interface{}) error {
	defer logElapsed("resource.tencentcloudenterprise_ngwaf_anti_info_leak.update")()
	defer inconsistentCheck(d, meta)()

	var (
		logId                               = getLogId(contextNil)
		modifyAntiInfoLeakRulesRequest      = ngwaf.NewModifyAntiInfoLeakRulesRequest()
		modifyAntiInfoLeakRuleStatusRequest = ngwaf.NewModifyAntiInfoLeakRuleStatusRequest()
	)
	...
	err := resource.Retry(writeRetryTimeout, func() *resource.RetryError {
		result, e := meta.(*TencentCloudClient).apiV3Conn.UseNgwafClient().ModifyAntiInfoLeakRules(modifyAntiInfoLeakRulesRequest)
		if e != nil {
			return retryError(e)
		} else {
...
func resourceTencentCloudNgwafAntiInfoLeakDelete(d *schema.ResourceData, meta interface{}) error {
	defer logElapsed("resource.tencentcloudenterprise_ngwaf_anti_info_leak.delete")()
	defer inconsistentCheck(d, meta)()
