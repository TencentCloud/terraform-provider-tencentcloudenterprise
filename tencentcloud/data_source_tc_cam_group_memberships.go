...
```hcl
	...
	data "tencentcloudenterprise_cam_group_memberships" "foo" {
...
func init() {
	registerDataDescriptionProvider("tencentcloudenterprise_cam_group_memberships", CNDescription{
		TerraformTypeCN: "CAM用户组成员列表",
		DescriptionCN:   "用于查询 CAM 用户组成员信息的列表。",
		AttributesCN: map[string]string{
			"group_id":        "用户组 ID",
			"membership_list": "成员列表",
			"user_ids":        "用户 ID 集合 (已弃用)",
			"user_names":      "用户名集合",
		},
	})
}
...
func dataSourceTencentCloudCamGroupMembershipsRead(d *schema.ResourceData, meta interface{}) error {
	defer logElapsed("data_source.tencentcloudenterprise_cam_group_memberships.read")()
